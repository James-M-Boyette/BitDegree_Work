...
const dbURI = 'mongodb+srv://testUser1984:student1234@sou7hernsaintmongodb.7x7o7.mongodb.net/myFirstDatabase?retryWrites=true&w=majority';
...
app.get("/randomJSON", (request, response) => {
    const quotes =  JSON.parse(fs.readFileSync("./public/quotes.json", (err, data) => {
        if(err){
            console.log(err);
        }
    ...
    // From original, FE randomizer
    const randomNumber = Math.floor(Math.random() * quotes.quotesJackHandey.length);
    console.log(randomNumber);
